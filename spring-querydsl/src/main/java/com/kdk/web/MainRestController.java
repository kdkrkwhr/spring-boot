...
public class MainRestController {
  ...
  @ApiOperation(value = "입력 API", tags = "JPA-QueryDSL")
  @ApiImplicitParams({
    @ApiImplicitParam
  ...
  @ApiOperation(value = "단일 조회 API", tags = "JPA-QueryDSL")
  @RequestMapping(value = "/select/one", method = RequestMethod.GET)
  public ResponseEntity<ResultVo> selectOne(String idx) {
    ResultVo result = ResultVo.builder().resultData(service.bookSelectOne(idx == null ? "0" : idx.trim())).build();
    return new ResponseEntity<>(result, HttpStatus.OK);
  }

  @ApiOperation(value = "목록 조회 API", tags = "JPA-QueryDSL")
  @RequestMapping(value = "/select/list", method = RequestMethod.GET)
  public ResponseEntity<ResultVo> selectList() {
    ResultVo result = ResultVo.builder().resultData(service.bookSelectList()).build();
    return new ResponseEntity<>(result, HttpStatus.OK);
  }
  ...
  @ApiOperation(value = "수정 API", tags = "JPA-QueryDSL")
  ...
  @ApiOperation(value = "삭제 API", tags = "JPA-QueryDSL")
  @ApiImplicitParams({
    @ApiImplicitParam
    (name = "idx", value = "조회 Index", required = true),
  })
